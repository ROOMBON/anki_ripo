...
object ZipHelper {
    ...
    /**
     * Scans the ZIP file for a 'repo_struct.json' at the root level or anywhere.
    ...
    /**
     * Reads a specific TXT file inside the ZIP and parses its lines into Question objects.
     */
    fun readAndParseTxtFile(context: Context, zipUri: Uri, filePath: String, importedKeys: Set<String>): List<Question> {
        val questions = mutableListOf<Question>()
        var tempFile: File? = null
        try {
            // First extract the ZIP temporarily or open as ZipFile
            tempFile = copyToTempFile(context, zipUri)
            ZipFile(tempFile).use { zipFile ->
                // Try finding the entry with the exact path or normalized path (slashes vs backslashes)
                val entryName = filePath.replace("\\", "/")
                var entry = zipFile.getEntry(entryName)
                if (entry == null) {
                    // Try looking up entry case-insensitively
    ...
    private fun scanMediaInZip(zip: ZipFile): Map<String, ZipEntry> {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (!entry.isDirectory) {
                ...
                // If it is in a media folder (like "מאגר השאלות/media/sound.mp3")
    ...
    private fun copyToTempFile(context: Context, uri: Uri): File {
        val tempFile = File.createTempFile("anki_repo_temp", ".zip", context.cacheDir)
